...
describe('CVM Fund Delivery FII identity and taxonomy', () => {
  ...
  it('ignores non-registry, formatted and padded CNPJs without fuzzy association', () => {
    const result = extract([
      createFundDeliveryFixtureRow({ CNPJ_Fundo_Classe: '99999999999999' }),
      createFundDeliveryFixtureRow({ CNPJ_Fundo_Classe: '12.005.956/0001-65' }),
      createFundDeliveryFixtureRow({ CNPJ_Fundo_Classe: ' 12005956000165 ' }),
    ])
    expect(result).toMatchObject({
      totalRows: 3,
      ignoredNonUniverseRows: 3,
      targetRows: 0,
    })
  })
})
...
describe('CVM Fund Delivery FII event mapping', () => {
  ...
  it('encodes source system safely without changing its provenance value', () => {
    const event = extract([
