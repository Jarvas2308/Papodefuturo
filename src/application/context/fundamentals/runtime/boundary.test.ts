...
describe('FundamentalsRuntimeV1 browser and financial boundaries', () => {
  ...
  it('is not imported by critical financial flows', () => {
    ...
    expect(source).not.toMatch(
      /fundamentals\/runtime|createFundamentalsRuntimeV1|useFundamentalsRuntime/
    )
  })

  it('contains no automatic Supabase or repository operation at module scope', () => {
    const source = Object.values(runtimeSources).join('\n')
    expect(source).not.toMatch(/void\s+createSupabaseFundamentalsRuntimeV1/)
