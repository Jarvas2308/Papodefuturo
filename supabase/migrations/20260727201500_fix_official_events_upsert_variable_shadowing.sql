-- A transactional audit of the real, applied official-events schema (run
-- with rollback, leaving no data behind) found that
-- upsert_official_asset_events_v1 has never successfully processed a single
-- record: the PL/pgSQL variables `event_id` and `deduplication_key` share
-- their names with real columns of `public.official_asset_events`. Inside
-- embedded SQL that references the table (`from public.official_asset_events
-- as stored where stored.event_id = event_id`), a bare, unqualified
-- `event_id` is ambiguous between the variable and the column. PostgreSQL's
-- default `plpgsql.variable_conflict = error` rejects this at execution time
-- with `42702: column reference "event_id" is ambiguous`.
--
-- This failure happens on the very first lookup performed for every record
-- in the batch, before the insert/update branch is even chosen. It blocked
-- both insert and update paths equally; it was never caught because the
-- Vitest suite never executes this function against a real Postgres
-- instance, and this was the first time it ran there.
--
-- Forward fix: rename the two colliding PL/pgSQL variables to `v_event_id`
-- and `v_deduplication_key`. JSON key literals such as `'event_id'`,
-- `'eventId'` and `'deduplication_key'` are untouched, as are already-
-- qualified column references such as `stored.event_id` and
-- `incoming.event_id`. No other function is affected: only this function
-- declares variables that collide with a table column name.
...
begin
  ...
  loop
    input_index := (item_json ->> 'inputIndex')::integer;
    incoming_record := item_json -> 'record';
    v_event_id := incoming_record ->> 'event_id';
    v_deduplication_key := incoming_record ->> 'deduplication_key';
    disposition := null;
    ...
    if working_records ? v_event_id then
      stored_record := working_records -> v_event_id;
    else
    ...
    if working_deduplication ? v_deduplication_key then
      deduplication_event_id := working_deduplication ->> v_deduplication_key;
    else
      select stored.event_id
      into deduplication_event_id
      from public.official_asset_events as stored
      where stored.deduplication_key = v_deduplication_key;
    end if;

    if stored_record is not null
      and stored_record ->> 'deduplication_key' <> v_deduplication_key
    then
      ...
      and deduplication_event_id <> v_event_id
    then
    ...
    result_items := result_items || pg_catalog.jsonb_build_array(
      pg_catalog.jsonb_build_object(
        ...
        'eventId', v_event_id,
        'deduplicationKey', v_deduplication_key,
